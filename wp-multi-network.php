...
Description: Adds a Networks panel for network admins to create and manipulate multiple networks.
Version: 0.1
...
/* This plugin originally created by David Dean */
...
if ( !function_exists( 'add_network' ) ) {
	function add_network( $domain, $path, $blog_name = NULL, $clone_network = NULL, $options_to_clone = NULL ) {

		if ( $blog_name == NULL )
			$blog_name = __( 'New Network Created' );
		...
		if ( !is_null( $clone_network ) && network_exists( $clone_network ) ) {

			$options_cache = array();

			switch_to_network( (int)$clone_network );
		...
		do_action( 'ms_add_network' , $new_site_id );
	...
	function update_network( $id, $domain, $path = '' ) {
		...
		if ( $blogs ) {
			foreach( $blogs as $blog ) {
				$domain = str_replace( $network->domain, $domain, $blog->domain );
				if( are_new_wpdb_funcs_available() ) {
					$wpdb->update(
						$wpdb->blogs,
						array(
				...
				foreach ( $options_list as $option_name ) {
					...
					if ( $option_value ) {
						...
						//$wpdb->query("UPDATE $optionTable SET option_value='$new_value' WHERE option_name='$option_name'");
					}
				}
			}
		}
		do_action( 'ms_update_network' , $id, array( 'domain'=>$network->domain, 'path'=>$network->path ) );
	}
}
...
if ( !function_exists( 'delete_network' ) ) {
	function delete_network( $id, $delete_blogs = false ) {
		...
		if ( $blogs && !$override )
			return new WP_Error( 'network_not_empty', __( 'Cannot delete network with blogs.' ) );
		...
		do_action( 'ms_delete_network' , $network );
	}
}
 ...
 * @param integer $new_site_id ID of destination network
 */
if ( !function_exists( 'move_blog' ) ) {
	function move_blog($blog_id, $new_site_id) {
		...
		if ( (int)$new_site_id == $blog->site_id )
			return true;

		$old_site_id = $blog->site_id;

		if ( ENABLE_NETWORK_ZERO && $blog->site_id == 0 ) {
		...
		if ( $new_site_id == 0 && ENABLE_NETWORK_ZERO ) {
		...
		if ( defined('VHOST') && VHOST == 'yes' ) {
			$ex_dom = substr( $blog->domain, 0, ( strpos( $blog->domain, '.' ) + 1 ) );
			$domain = $ex_dom . $new_network->domain;
		} else {
			$domain = $new_network->domain;
		}
		$path = $new_network->path . substr( $blog->path, strlen( $old_network->path ) );
		...
		if ( are_new_wpdb_funcs_available() ) {
			...
			$update_result = $query = "UPDATE {$wpdb->blogs} SET site_id=" . $new_network->id . ", domain='" . $domain . "', path='" . $path . "' WHERE blog_id=" . $blog->blog_id;
			$wpdb->query($query);
		...
		$options_table = $wpdb->get_blog_prefix( $blog->blog_id ) . "options";
		...
		foreach($options_list as $option_name) {
			...
			update_blog_option( $blog->blog_id, $option_name,$new_value );
//			$query = "UPDATE $options_table SET option_value='$new_value' WHERE option_name='$option_name'";
//			$wpdb->query($query);
		}

		do_action( 'ms_move_blog' , $blog_id, $old_site_id, $new_site_id );
	}
}
...
?>
							<th scope="row" class="check-column" style="width: auto"><input type='checkbox' id='<?php echo $blog[ 'id' ] ?>' name='allnetworks[]' value='<?php echo $blog[ 'id' ] ?>'><label for='<?php echo $blog[ 'id' ] ?>'></label</th>
<?php
					else :
?>
							<th></th>
...
<?php
				...
				case 'domain':
?>
							<td><?php echo $blog['domain'] ?>
<?php
	$actions	= array();
	$actions[]	= '<a class="edit" href="' . $query_str . "&amp;action=assignblogs&amp;id=" . $blog['id'] . '" title="' . __('Assign blogs to this network') . '">' . __( 'Assign Blogs' ) . '</a>';
	$actions[]	= '<a class="edit" href="' . $query_str . "&amp;action=editnetwork&amp;id=" .  $blog['id'] . '" title="' . __('Edit this network') . '">' . __('Edit') . '</a>';
	if ( $blog['blogs'] == 0 || $blog['id'] != 1 )
		$actions[] = '<a class="delete" href="' . $query_str . "&amp;action=deletenetwork&amp;id=" . $blog['id'] . '" title="' . __('Delete this network') . '">' . __('Delete') . '</a>';
...
<?php
				...
				case 'blogs':
?>
							<td valign='top'><a href="http://<?php echo $blog['domain'] . $blog['blog_path'];?>wp-admin/ms-sites.php" title="<?php _e('Blogs on this network'); ?>"><?php echo $blog['blogs'] ?></a></td>
<?php
					break;

				default:
?>
							<td valign='top'><?php do_action('manage_networks_custom_column', $column_name, $blog['id']); ?></td>
<?php
					break;
			}
		}
...
<?php if ( $network_navigation ) echo "<div class='tablenav-pages'>$network_navigation</div>"; ?>
		...
		<div id="col-left">
			...
			<p><?php _e('A blog will be created at the root of the new network'); ?>.</p>
				<form method="POST" action="<?php echo $_SERVER['REQUEST_URI'] . "&amp;action=addnetwork"; ?>">
					<table class="form-table">
						...
						<tr><th scope="row"><label for="newBlog"><?php _e('Blog Name'); ?>:</label></th><td><input type="text" name="newBlog" id="newBlog" title="<?php _e('The name for the new network\'s blog'); ?>" /></td></tr>
					</table>
...
<?php
	...
	function move_blog_page() {
		...
		if ( isset( $_POST['move'] ) && isset( $_GET['blog_id'] ) ) {
			...
			$table_name = $wpdb->get_blog_prefix( $blog->blog_id ) . "options";
		...
		if ( isset( $_POST['reassign'] ) && isset( $_GET['id'] ) ) {
			...
			if ( ENABLE_NETWORK_ZERO ) {
				foreach( $current_blogs as $current_blog ) {
					if ( !in_array( $current_blog->blog_id, $blogs ) )
						move_blog( $current_blog->blog_id, 0 );
			...
			foreach( $blogs as $key => $blog ) {
				$table_name = $wpdb->get_blog_prefix( $blog->blog_id ) . "options";
				$blog_name = $wpdb->get_row("SELECT * FROM $table_name WHERE option_name='blogname'");
...
?>
			<div id="icon-tools" class="icon32"></div>
			<h2><?php _e ('Networks') ?></h2>
			<h3><?php _e('Assign Blogs to'); ?>: http://<?php echo $network->domain . $network->path ?></h3>
			<noscript>
...
<?php
	foreach($blogs as $blog) {
		if($blog->site_id != $network->id) echo '<option value="' . $blog->blog_id . '">' . $blog->name  . ' (' . $blog->domain . ')</option>';
	}
?>
							</select>
						</td>
	...
	function add_network_page() {
		...
		if ( isset( $_POST['add'] ) && isset( $_POST['domain'] ) && isset( $_POST['path'] ) ) {
			...
			$result = add_network(
				$_POST['domain'],
				$_POST['path'],
				(isset($_POST['newBlog']) ? $_POST['newBlog'] : __('New Network Created') ) ,
